...
;(function(root, factory) {
	...
	jedfn.init = function(){
		var that = this;
		var opts = that.opts;
		// jet.boxCell = opts.contThis?opts.contThis:"#jedatebox";
		var zIndex = opts.zIndex == undefined ? 2099 : opts.zIndex;
		var isinitVal = (opts.isinitVal == undefined || opts.isinitVal == false) ? false : true;
		// jet.boxCell.replace(/\#/g,"")
		var createDiv = $("<div  class='jedatebox " + (opts.skinCell || config.skinCell)+"'></div");
		...
		if (jet.isBool(opts.insTrigger)) {
			that.valCell.off().on("click", function (ev) {
				ev.stopPropagation();
				...
				// 判断添加日历位置
				if(opts.site){
 				  $(opts.contId).append(createDiv);
				}else{
				  $("body").append(createDiv);
				}
				
				jet.boxCell = that.valCell.siblings("div.jedatebox");
				that.setHtml(opts);
			});
		}else {
			jet.format = opts.format || config.format;
	...
	jedfn.setHtml = function(opts){
		...
		//是否显示确定按钮
		jet.isBool(opts.isConfirm) ? "" : jet.isShow(boxCell.find(".jedatebot .jedateok"), false);

		//是否显示底部按钮与时间栏
		jet.isBool(opts.isFoot) ? "" : jet.isShow(boxCell.find(".jedatebot "), false);
	...
	jedfn.createDaysHtml = function(ys, ms, opts){
		var that = this;
		var elemCell = that.valCell;
		var boxCell = that.valCell.siblings("div.jedatebox");
		var year = parseInt(ys);
		var month = parseInt(ms);
		var dateHtml = "",count = 0;
	...
	jedfn.chooseDays = function(opts) {
		...
		var that = this, elemCell = that.valCell, boxCell = jet.boxCell;
		boxCell.find(".jedaul li").off().on("click", function(ev) {
		...
		if(opts.festival) {
			//鼠标进入提示框出现 
			boxCell.find(".jedaul li").off().on("mouseover", function () {
			...
			}).off().on( "mouseout", function () { //鼠标移除提示框消失
	...
	jedfn.chooseYM = function(opts) {
		var that = this;
		var boxCell = jet.boxCell;
		var jetopym = boxCell.find(".jedatetopym");
		var jedateyy = boxCell.find(".jedateyy");
		var jedatemm = boxCell.find(".jedatemm");
		var jedateyear = boxCell.find(".jedateyy .jedateyear");
		var jedatemonth = boxCell.find(".jedatemm .jedatemonth");
		var mchri = boxCell.find(".jedateymchri");
		var mchle = boxCell.find(".jedateymchle");
		var ishhmmss = jet.checkFormat(jet.format).substring(0, 5) == "hh-mm" ? true :false;
		var minArr = jet.minDate.match(ymdMacth);
		var minNum = minArr[0] + minArr[1];
		var maxArr = jet.maxDate.match(ymdMacth);
		var maxNum = maxArr[0] + maxArr[1];
		//循环生成年
		...
		function clickLiYears(year) {
			boxCell.find(".ymdropul li").off().on("click", function(ev) {
				var _this = $(this);
				var Years = _this.attr("yy");
				var Months = parseInt(jedatemonth.attr("month"));
		...
		//下拉选择年
		!ishhmmss && jedateyy.off().on("click", function() {
			var yythat = $(this);
			var YMlen = parseInt(yythat.attr("ym"));
			var yearAttr = parseInt(jedateyear.attr("year"));
			eachYearMonth(yearAttr, YMlen);
			clickLiYears(jedateyear);
		});
		//下拉选择月
		!ishhmmss && jedatemm.off().on("click", function() {
		...
		//关闭下拉选择
		boxCell.find(".jedateymchok").off().on("click", function(ev) {
			ev.stopPropagation();
			jet.isShow(jetopym,false);
		});
	...
	jedfn.ymPremNextEvents = function(opts){
		...
		//选择年月
		ymCls.off().on("click", function (ev) {
	...
	jedfn.events = function(tmsArr,opts){
		...
		if (!isYYMM) {
			...
			var hmsStr = that.setStrhms(opts), hmsevents = function(hmsArr) {
				...
				$.each([ "hours", "minutes", "seconds" ], function(i, hms) {
					...
					onhmsPCls.off().on("click", function() {
						var _this = $(this);
						if (_this.hasClass("disabled")) return;
			...
			if (ishhmmss) {
				...
				boxCell.find(".jedatehms").off().on("click", function() {
					if (boxCell.find(".jedateprophms").css("display") !== "block") hmsevents([ hs, ms, ss ]);
					//关闭时分秒层
					boxCell.find(".jedateprophms .jedatehmsclose").off().on("click", function() {
						jet.isShow(boxCell.find(".jedateprophms"), false);
					});
				});
			}
			//今天按钮设置日期时间
			boxCell.find(".jedatebot .jedatetodaymonth").off().on("click", function() {
			...
			that.ymPremNextEvents(opts);
			//本月按钮设置日期时间
			boxCell.find(".jedatebot .jedatetodaymonth").off().on("click", function(ev) {
				ev.stopPropagation();
		...
		//检查时间输入值，并对应到相应位置
		boxCell.find(".jedatehms input").off().on("keyup", function() {
		...
		//清空按钮清空日期时间
		boxCell.find(".jedatebot .jedateclear").off().on("click", function(ev) {
			ev.stopPropagation();
		...
		//确认按钮设置日期时间
		boxCell.find(".jedatebot .jedateok").off().on("click", function(ev) {
			ev.stopPropagation();
		...
		//点击空白处隐藏
		if(opts.clickGap){
			$(document).off().on("mouseup", function(ev) {
				ev.stopPropagation();
				var box = jet.boxCell;
				if (box && box.css("display") !== "none")  box.remove();
			});
			jet.boxCell.off().on("mouseup", function(ev) {
				ev.stopPropagation();
			});	
		}
	};
